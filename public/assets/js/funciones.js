...
function pruebaCon(tipo,server,name,user,pass) {
  data = new FormData();
  data.append('tag', tag);
  ...
  if((tag != "") && (tipo != "") && (server != "") && (name != "") && (user != "")){
    ...
    alertae('Hubo un error en la selección de campos');
  }
}

function loadTables(tipo,server,name,user,pass) {
  data = new FormData();
  data.append('tipo', tipo);
  data.append('server', server);
  data.append('name', name);
  ...
  if((tipo != "") && (server != "") && (name != "") && (user != "")){
    $.ajax({
      url: "controller/dbsLoadTables",
      type: "POST",
      data:data,
      enctype: 'multipart/form-data',
      processData: false,  // tell jQuery not to process the data
...
function loadColumns(tipo,server,name,user,pass,tabla) {
  data = new FormData();
  data.append('tipo', tipo);
  data.append('server', server);
  data.append('name', name);
  ...
  if((tipo != "") && (server != "") && (name != "") && (user != "") && (tabla != "")){
    $.ajax({
      url: "controller/dbsLoadColumns",
      type: "POST",
      data:data,
      enctype: 'multipart/form-data',
      processData: false,  // tell jQuery not to process the data
...
function preview(tipo,server,name,user,pass,tabla,columnas) {
  data = new FormData();
  data.append('tipo', tipo);
  data.append('server', server);
  data.append('name', name);
  ...
  if((tipo != "") && (server != "") && (name != "") && (user != "") && (tabla != "") && (columnas != "")){
    $.ajax({
      ...
      success: function(dataResult){
        ...
        if(dataResult.statusCode==200){
          ...
          var cols = columnas.toString().split(",");
          $('#headTable').html('');
          $('#Contenido').html('');
          for (var i = 0; i < cols.length; i++) {
            $('#headTable').append('<th>'+cols[i]+'</th>');
          }
...
function delDB(idC) {
  Swal.fire({
    title: '¿Estás seguro de eliminar la conexión a esta base de datos?',
    text: "No podrás revertir esta acción, todos los reportes asociados a esta conexión tendrán que ser re-configurados",
    icon: 'warning',
    showCancelButton: true,
    confirmButtonColor: '#3085d6',
    cancelButtonColor: '#d33000',
    ...
    if (result.isConfirmed) {
      $.post('controller/delDB', { id: idC}, 
        function(returnedData){
          ...
          alertaNoti("La conexión a esta base de datos se ha eliminado con exito");
      }).fail(function(){
          alertae("error");
      });
    }
...
function dataModal(id,icon,value) {
  $('#categoria').val(value);
...
function dataModalDB(id,tag,tipo,server,nombre,user,pass) {
  $('#tag').val(tag);
  $('#tipo').val(tipo);
  $('#tipo').change();
  $('#server').val(server);
